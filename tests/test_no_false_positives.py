...
from nirjas.main import SOURCES
...
def test_every_code_language_has_a_sentinel_fixture():
    """The rule holds for one language only by accident unless all of them run it.

    Two grammars were caught nesting a comment node inside a quoted construct,
    and there is no reason to think those two are special -- each grammar is an
    independent project making its own parse-tree choices. So the check is
    required everywhere rather than wherever somebody happened to write a
    fixture, and adding a language without one fails here.

    Keyed off SOURCES rather than EXTRACTORS: `text` has an extractor but no
    strings and no comments, so there is nothing for it to get wrong.
    """

    covered = {nirjas_name_from_path(str(fixture.values[0])) for fixture in PARAMS}
    missing = sorted(set(SOURCES) - covered)
    assert not missing, (
        f"languages with no string-literal false-positive fixture: {missing}; "
        f"add one carrying {STRING_ONLY_SENTINEL!r} inside a string and "
        f"{REAL_COMMENT_SENTINEL!r} in a real comment"
    )
