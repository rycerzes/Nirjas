...
- **`tests/data/corpus`** — three real-world files per language, vendored
  ...
  no hand-written fixture thinks to include. Origin, pinned commit and licence
  for all of them live in `tests/data/corpus/PROVENANCE.json`, grouped by
  upstream. Run just these with `pytest -m corpus`.
